...
void GLVertexArray::AttachBuffer(GLBuffer* buffer, const BufferLayout& layout)
{
  ...
  {
    glVertexAttribPointer(m_CurrentAttrib, ShaderDataTypeCount(element.Type),
                          GLenumFromShaderDataType(element.Type), element.Normalized, layout.Stride,
                          (void*)element.Offset);
...
template <typename T> void HashCombine(std::size_t& Seed, const T& Val) noexcept
{
  Seed ^= std::hash<T>()(Val) + 0x9e3779b9 + (Seed << 6) + (Seed >> 2);
}

// template <typename FirstArgType, typename... RestArgsType>
// void HashCombine(std::size_t &Seed, const FirstArgType &FirstArg, const RestArgsType
// &...RestArgs) noexcept
// {
